...
import { FileText, File, Link, Download, Trash2, Edit2, Save, X, ExternalLink, Eye, Calendar, User, HardDrive, Tag, Folder } from 'lucide-react';
...
interface ProjectDocumentListProps {
  ...
  onDocumentUpdated: (document: ProjectDocument) => void;
  readonly?: boolean;
}

// Document category options
const DOCUMENT_CATEGORIES = [
  { value: 'requirements', label: 'Requirements', color: 'blue' },
  { value: 'design', label: 'Design', color: 'purple' },
  { value: 'specification', label: 'Specification', color: 'green' },
  { value: 'reference', label: 'Reference', color: 'yellow' },
  { value: 'media', label: 'Media', color: 'pink' },
...
export default function ProjectDocumentList({
  ...
  const handleDownload = async (document: ProjectDocument) => {
    ...
    try {
      const response = await fetch(`/api/project-documents/${document.id}`);
      if (response.ok) {
  ...
  const handleDelete = async (document: ProjectDocument) => {
    const confirmMessage = document.url
      ? `Remove URL "${document.originalFilename}" from this project?`
  ...
  const handleEditDescription = (document: ProjectDocument) => {
    setEditingDescription(document.id);
    setEditedDescription(document.description || '');
  };

  const handleSaveDescription = async (document: ProjectDocument) => {
    try {
      const response = await fetch(`/api/project-documents/${document.id}`, {
        method: 'PATCH',
  ...
  const handleCategoryChange = async (document: ProjectDocument, newCategory: string) => {
    try {
      const response = await fetch(`/api/project-documents/${document.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      alert('Failed to update category. Please check your network connection.');
    }
  };
  ...
  if (documents.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500 dark:text-gray-400">
        ...
        <p className="text-sm mt-2">Upload documents to provide context for this project</p>
      </div>
    );
  }
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    {editingCategory === document.id ? (
                      <select
                        value={document.category}
                        onChange={(e) => handleCategoryChange(document, e.target.value)}
                        onBlur={() => setEditingCategory(null)}
                        className="text-xs px-2 py-1 rounded-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        autoFocus
                      >
                      ...
                      <button
                        onClick={() => !readonly && setEditingCategory(document.id)}
                        className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full ${getCategoryBadgeClass(document.category)} ${
                          readonly ? 'cursor-default' : 'hover:opacity-80 cursor-pointer'
                        }`}
                      >
                        <Folder className="w-3 h-3" />
                        {DOCUMENT_CATEGORIES.find(c => c.value === document.category)?.label || 'Other'}
                      </button>
                    )}
